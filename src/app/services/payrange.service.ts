...
import PayRange from '@models/payrange.schema';
...
export default class PayRangeService extends Service {
  private payrangeModel: any;
  ...
  async count(): Promise<Response<any>> {
    try {
      const result = await this.payrangeModel.countDocuments();
      if (!result) {
  ...
  async list(): Promise<Response<any>> {
    try {
      const result = await this.payrangeModel.find();
      if (!result) {
        return new Response<any>(true, 200, 'Record not available', result);
      }
  ...
  async retrieve(pid: string): Promise<Response<any>> {
    try {
      ...
      const record = await this.payrangeModel.findById(pid);
      if (!record) {
  ...
  async retrieveByPayRange(name: string): Promise<any> {
    try {
      const records = await this.payrangeModel.findOne({ name: name });
      return records;
    } catch (error: any) {
      return error;
    }
  ...
  async create(data: any): Promise<Response<any>> {
    try {
      const payrange = new PayRange();
      payrange.range   = data.range;
      payrange.createdAt = new Date();
      payrange.createdBy = data.createdBy
      payrange.createdFrom = data.ip
      const result = await payrange.save();
  ...
  async update(pid: string, data: any): Promise<Response<any>> {
    try {
      ...
      const payrange = await this.payrangeModel.findById(pid);
      if (!payrange) {
        return new Response<any>(true, 200, 'Record not available', payrange);
      }
      if (data.range) {
        payrange.range = data.range;
      }
      payrange.updatedAt = new Date()
      payrange.updatedBy = data.updatedBy
      payrange.updatedFrom = data.ip
      const result = await payrange.save();
  ...
  async delete(pid: string,data:any): Promise<Response<any>> {
    try {
      ...
      payrange.deletedAt = moment().toDate();
      payrange.deletedAt = moment().toDate(); // Set the deleted_at field to the current timestamp
      payrange.deleteBy = data.deleteBy;
      payrange.deleteFrom = data.ip;
      const result = await payrange.save();
    ...
    try {
      ...
      const [records, totalCount] = await Promise.all([
        this.payrangeModel.find()
          ...
          .skip(skip)
          .limit(limit),
        this.payrangeModel.countDocuments(searchQuery),
      ]);
  ...
  async searchPayRanges(query: any): Promise<Response<any>> {
    try {
      const searchOptions = {
      ...
      const result = await this.searchEngine.search('payrange', query, searchOptions);
